      ...
      <v-card>
        <v-card-title>
          <v-avatar size="36px" slot="activator">
            <img :src="findAvatar(news.author)" alt="" />
          </v-avatar>
          <span :title="news.author" style="padding-left:10px">{{
            news.author ? chrinkAuthor(news.author) : 'Auteur Inconnu'
          }}</span>
          <!-- Photo Author -->
          <!-- Author -->
          <!-- Photo Author -->
        ...
        <v-card-title :color="getColor(news)" style="padding-top:0px">
          <div>
            <span class="subheading" :color="getColor(news)">{{
              news.resume
            }}</span>
          </div>
        </v-card-title>
